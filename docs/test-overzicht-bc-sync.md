...
| 11 juni | **Live-test 2 echte orders (12:25 UTC): VERDWENEN — open punt 2 gereproduceerd** met verse PO's (4002950522/21) én `BRATRA-NL` | HTTP 201, DLQ leeg, geen buffer-rijen. Daarmee vallen duplicaat-PO én legalEntity af als verklaring. Envelope structureel identiek aan de geslaagde happy-payload; enige waarde-verschillen: lege `distributionCenter.code` (""), ander contractnummer, 2 orders/envelope, echte EAN's. Volledig verslag + exacte envelope: `verdwenen-orders-envelope-2026-06-11.md` (deze map), gedeeld met Leo |
| 11 juni | **Leesrechten Table 55001 toegekend (Wesley) — verifier werkt.** Re-send van de 2 live-orders geverifieerd via de API | Verifier draait zonder 403; uitkomst `notFound: 2` = API-bewijs voor punt 2. Happy-rij via API bevestigd: status Done, VO26-00160. Buffer-API geeft alle velden (customerNo, errorMessage, receivedAt, processedAt) voor verder onderzoek |
...
|---|---|---|---|---|
| 1 | Read-rechten op buffer-tabel (Table 55001, Bratra SO Buffer) voor onze M2M-app | 23 mei | 4 juni | **OPGELOST 11 juni (Wesley).** Verifier leest de buffer-API succesvol; happy-rij via API bevestigd (Done, VO26-00160). De `not_found`-uitkomst van de verifier is daarmee hard API-bewijs voor punt 2 |
| 2 | Op 4 juni verstuurde orders (11 stuks, HTTP 201, geen DLQ) zijn bij visuele controle niet zichtbaar in de buffer-tabel | 4 juni (testverslag) | 9 juni | Open, maar AANGESCHERPT door de geslaagde happy-path test van 11 juni: een order met een vers PO-nummer komt wél in de buffer en gaat naar Done. Twee verschillen met 4 juni: (a) de 11 orders waren re-dispatches van PO's die al als sales order bestaan (guide zegt: zou Error-rij "already exists" moeten geven, maar er verschijnt niets), en (b) de dispatcher stuurde `legalEntity: BRATRA-NONFOOD`, de geslaagde test `BRATRA-NL` — zie punt 3 |
| 3 | **Exacte legalEntity-waarden per Bratra-bedrijf.** Gevraagd op 20 mei ("laat me de exacte waarden nog even weten"), nooit beantwoord. Onze dispatcher stuurt nu `BRATRA-NONFOOD` (non-food) en `BRATRA-PRODUCTS` (Pet Products) op basis van Wesley's mail van 20 mei; het enige bevestigd werkende is `BRATRA-NL` (Postman-collectie, guide, happy-test 11 juni). Vraag: welke waarde routeert naar welke BC-company (Food / Non-Food / Pet Products), en wat gebeurt er met een onbekende waarde (stil genegeerd of DLQ)? | 20 mei | 11 juni (nieuw aangescherpt) | Open — mogelijk mede-oorzaak van punt 2 |
